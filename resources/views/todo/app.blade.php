                            ...
                            @forelse ($data as $item)
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <input type="text" class="form-control edit-input" style="display: none;"
                                        value="{{ $item->task }}">
                                    <div class="btn-group">
                                        <form action="{{ route('todo.delete', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus task?')">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <form action="{{ route('todo.update', $item->id) }}" method="POST">
                                        ...
                                        <div class="d-flex">
                                            <div class="radio px-2">
                                                <label>
                                                    <input type="radio" value="1" name="is_done" {{ $item->is_done ? 'checked' : '' }}> Selesai
                                                </label>
                                            </div>
                                            <div class="radio">
                                                <label>
                                                    <input type="radio" value="0" name="is_done" {{ $item->is_done ? '' : 'checked' }}> Belum
                                                </label>
                                            </div>
                                        </div>
                                    </form>
